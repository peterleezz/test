...
<div class="main-container" id="main-container">
  ...
  <div class="row">
    <div class="col-xs-12">
      <form class="form-inline" role="form" action="<?php echo U('Finance/McReport/queryReport');?>" method="post" id="finance_mc_report" >
    ...
    <div class="col-xs-12">
      <table id="finance_mc_grid"></table>
      <div id="finance_mc_grid_pager"></div>
    </div>
...
$("#finance_mc_report").submit(function(){   
         var self = $(this);  
        ...
        end_time=$("#end_time").val();
         ...
         jQuery("#finance_mc_grid").jqGrid('setGridParam',{postData:self.serialize(),url:self.attr("action"),page:1,datatype:'json'}).trigger("reloadGrid"); 
         return false;
     });
    ...
    var grid_selector = "#finance_mc_grid";
    var pager_selector = "#finance_mc_grid_pager";

    
                jQuery(grid_selector).jqGrid({
                    url:"/Finance/McReport/queryReport",                 
                   mtype:"POST",        
                  datatype: "local",
                  height: 'auto', 
                  width:"auto",
                    colNames:['销售人员','合同数','应缴收入','实际收入','欠款','classify'],
                    colModel:[ 
                      
                        {name:'name_cn',index:'name_cn',formatter : function(value, options, rData){  
                            if(end_time=="")end_time="0";
                                            return "<a target='_blank' href='/Finance/McReport/detail?type="+rData.type+"&mc_id="+rData.mc_id+"&start_time="+start_time+"&end_time="+end_time+"&reporttype="+type+"'>"+value+"</a>"
                         }}, 
