...
<div class="main-container" id="main-container">
  ...
  <div class="main-container-inner">
	...
	<h4 class="green"> <i class="icon-hand-right icon-animated-hand-pointer blue"></i>
		高峰时段，用于禁止普通会员在这些时段入馆！
	</h4>
	  
	<div class="row">
		<div class="col-xs-12">
			<div class="table-responsive">
				<table id="peak_table" class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							<th>时段名</th>
							<!-- <th>会所</th>						  -->
					...
					<tbody>
						<?php if(is_array($peaks)): $i = 0; $__LIST__ = $peaks;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$peak): $mod = ($i % 2 );++$i;?><tr>
								<td><?php echo ($peak["peak_name"]); ?></td>
								<!-- <td>
									<a href="/Brand/Shop/clubinfo?id=<?php echo ($peak["club_id"]); ?>"><?php echo ($peak["club"]["club_name"]); ?></a>
								</td> -->
								<td>
								<?php if(is_array($peak["peak_time"])): $k = 0; $__LIST__ = $peak["peak_time"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ptime): $mod = ($k % 2 );++$k; if(1 != $k): ?>&&<?php endif; ?>
								<?php $arr=array("","周一","周二","周三","周四","周五","周六","周日");echo $arr[$ptime['week']] ?>
									
									   <?php echo ($ptime["start_time"]); ?>-<?php echo ($ptime["end_time"]); endforeach; endif; else: echo "" ;endif; ?>								
								</td> 
								<td><?php echo ($peak["create_time"]); ?></td>
								<td>
									<div class="action-buttons">
										<a class="green" href="<?php echo U('Brand/Shop/showEditPeak?id='.$peak['id']);?>">
											<i class="icon-pencil bigger-130"></i>
										</a>

										<a class="red" href="javascript:void(0)" onclick="delPeak(<?php echo ($peak["id"]); ?>,this)">
