	...
	<div class="row">
		<div class="col-xs-12">
			<div class="row">
				...
				<?php $headerColor=array("","header-color-red3","header-color-orange","header-color-blue","header-color-green","header-color-grey");$footColor=array("","btn-danger","btn-warning","btn-primary","btn-success","btn-grey"); ?>
					<?php if(is_array($heroes)): $i = 0; $__LIST__ = $heroes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hero): $mod = ($i % 2 );++$i;?><div class="pricing-span">
							<div class="widget-box pricing-box-small">
								<div class="widget-header <?php echo $headerColor[$i%6] ?>">
									<h5 class="bigger lighter"><?php echo ($hero["name"]); ?></h5>
								</div>

								<div class="widget-body">
									<div class="widget-main no-padding">
										<ul class="list-unstyled list-striped pricing-table">
											<li><?php echo ($hero["protential"]); ?></li>
											<li><?php echo ($hero["transform"]); ?></li>
											<li><?php echo ($hero["invit"]); ?></li>
											<li>￥<?php echo ($hero["cardsale"]); ?></li> 
											<li>￥<?php echo ($hero["goodssale"]); ?></li> 
										</ul> 
									</div>

									<div>
										<a href="javascript:void(0)" class="btn btn-block btn-sm  <?php echo $footColor[$i%6] ?>">
											<span>第<?php echo ($i); ?>名</span>
										</a>
									</div>
								</div>
							</div>
...
$(function(){ 
    ...
    var pager_selector = "#pager";
                ...
                jQuery(grid_selector).jqGrid({
                  ...
                  height: 'auto', 
                  width:"auto",
                    colNames:['销售人员','当日新增潜客','当日销售额','累计新增潜客','累计销售额','classify'],
                    colModel:[ 
                      
                        {name:'name_cn',index:'name_cn',sortable:false}, 
                       {name:'potential_d',index:'potential_d',summaryType:'sum',summaryTpl:'<b>总计: {0}</b>'},  
                       {name:'cardsale_d',index:'cardsale_d',summaryType:'sum',summaryTpl:'<b>总计: ￥{0}</b>'},  
                      {name:'potential_all',index:'potential_all',summaryType:'sum',summaryTpl:'<b>总计: {0}</b>'},    
                       {name:'cardsale_all',index:'cardsale_all',summaryType:'sum',summaryTpl:'<b>总计: ￥{0}</b>'},   
                         {name:'classify',index:'classify',hidden:true,sortable:false},
                    ],       
