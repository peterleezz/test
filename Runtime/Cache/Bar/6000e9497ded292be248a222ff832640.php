...
<head>
    ...
    <style type="text/css">
        ...
        .name,.name_r{width:60%;}
        .num,.num_r{width:20%;}
        .total,.total_r{width:20%;}
...
<body>
    <div class="xiaopiao">
        ...
        <div class="head2">
            ...
            <?php if(is_array($order)): $i = 0; $__LIST__ = $order;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($i % 2 );++$i;?><div class="name_r"><?php echo ($good["goods_name"]); ?></div>
                <div class="num_r">x<?php echo ($good["number"]); ?></div>
                   <div class="total_r">￥<?php echo ($good['price'] * $good['number']); ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
            <br>
            <hr style="border:1px dotted #000">
            <div>
             <div style="text-align:right ; ">应收￥<?php echo ($bill['price']); ?>.00</div>
        </div>

            <div style="text-align:right ; ">实收￥<?php echo ($history['cash']+$history['pos']+$history['check']+$history['network']+$history['netbank']); ?>.00</div>
        </div></div>
           <hr>
            <!-- <div class="head1">
                <div class="name">品名</div>
