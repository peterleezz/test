...
<div class="main-container" id="main-container">
  ...
  <div class="main-container-inner">
	...
	<ul class="breadcrumb">
		...
		<li class="active">渠道经理</li>
		<li class="active">渠道计划</li>
	...
	<form class="form-inline" role="form" action="<?php echo U('Channelmanager/Statistics/query');?>" method="post" id="query_channel_statistics" style="margin-bottom:10px">

	
	<div class="form-group">
			<label  class="sr-only" for="user_id">time：</label>
			<select name="user_id" id="user_id" class="form-control">
				<option value="0" selected="true">所有人员</option>
		...
		<div class="form-group st">
			<label  for="start_month">月份时间:</label>
			<input type="text" class="date_ym form-control" name="start_month"  id="start_month"></div>
		  

		  <div class="form-group et" style="display:none">
			<label  for="start_time">起时时间:</label>
			<input type="text" class="date_ymd form-control" name="start_time"  id="end_time"></div>
		<div class="form-group et"   style="display:none">
			<label  for="end_time">终止时间:</label>
			<input type="text" class="date_ymd form-control" name="end_time"  id="end_time"></div>
	...
	<table class="table">		
        	...
        	<tbody>  
        	  <tr  class="info" ><td colspan="9" align="center">本月数据统计 </td></tr>
        	<?php if(is_array($statistics)): $k = 0; $__LIST__ = $statistics;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$st): $mod = ($k % 2 );++$k;?><tr class="warning">  
					      				<td><?php echo ($k); ?></td>
					        			<td><?php echo ($st["name"]); ?></td>
					        			<td><?php echo ($st["level"]); ?></td>
					        			<td><?php echo ($st["protential"]); ?></td>
					        			<td><?php echo ($st["mcount"]); ?></td>
					        			<td> <?php if($st["protential"] == 0): ?>0<?php else: ?>  <?php echo ($st['mcount']*100/$st['protential']); endif; ?> %</td>

					        			<td><?php echo ($st["protential_total"]); ?></td>
					        			<td><?php echo ($st["mcount_total"]); ?></td>
					        			<td> <?php if($st["protential_total"] == 0): ?>0<?php else: ?>  <?php echo ($st['mcount_total']*100/$st['protential_total']); endif; ?> %</td>
