  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form class="form-horizontal" role="form" id="modalform" action="<?php echo U('Tuanmanager/Class/edit');?>">
...
var id = $("#id").val();
  if(id==0)//如果是添加
  {
     ...
     for(var i=0;i<5;i++)
     {
        ...
        $.ajax({
            type:"post", 
            url:"<?php echo U('Tuanmanager/Class/editone');?>",
            dataType:"json",
...
function save()
{
 ...
 var b = JSON.stringify(arr); 
	 ...
	 $.ajax({
    type:"post", 
    url:"<?php echo U('Tuanmanager/Class/edit');?>",
    dataType:"json",
...
function delid(id)
{
	 $.ajax({
    type:"post",
    url:"<?php echo U('Tuanmanager/Class/del');?>",
    dataType:"json",
    data:{id:id},
    success:function(data,textStatus){
    		 $(".modal").modal("hide");
	...
	$(document).ready(function() { 
	 ...
	 calendar = $('#calendar').fullCalendar({
        ...
        $("#start_time").val(start.format('yyyy-MM-dd hh:mm:ss'));
			...
			editable: true,
				events: {
				url:'<?php echo U("Tuanmanager/Class/getjson");?>',
				data: function() {
				 return {
                	user_id: $("#user_id").val()
            	};
