...
class EmployeeController extends BaseController {
	public  function indexAction()
	{
		...
		$this->display("Brand@Employee:index");
	}
	   ...
	   if (!$userModel->validate($rules)->create()){      
		    ...
		    $id = $userModel->add();
			 if (!$extensionModel->create()){      
			    ...
			    foreach ($roles as $key => $value) {
			    	M("FinanceClub")->data(array("user_id"=>$id,"club_id"=>$value))->add();	    
			    }
			}
		}
	...
	public function delAction($ids)
	{
		...
		foreach ($ids as $key => $id) {
			...
			if($id==is_user_login())
			{
				$response=array("success"=>false,"message"=>'不能删除自己帐号！',"new_id"=>$id);
				$this->ajaxReturn($response);
			}
			M("AuthGroupAccess")->where("uid=$id")->delete(); 
			M("User")->where("id=$id")->setField("invalid",0);
			M("MemberBasic")->where("mc_id=$id")->setField("mc_id",0);
			M("MemberBasic")->where("pt_id=$id")->setField("pt_id",0);
		}
	...
	{	
		...
		$map = array();
		$brand_id=get_brand_id();
		$sql="select * from `yoga_user`,`yoga_user_extension` where yoga_user.id=yoga_user_extension.id and `brand_id`=$brand_id ";		
		...
		if(!empty($name_cn))
		{
			// $map["name_cn"]=array("like","%$name_cn%");
			$sql.=" and name_cn like '"."%$name_cn%'";
		}
		$name_en = I("name_en");
		if(!empty($name_en))
		{
			// $map["name_en"]=array("like","%$name_en%");
			...
			$map[]="yoga_user.id=yoga_user_extension.id";
			$model = M("User");
			$brand_id = get_brand_id(); 

			// $employees = $model->table("yoga_user,yoga_user_extension")->where($map)->limit("$start,$limit")->buildSql();
			$employees =$model->query($sql);

			// $count = 	$model->table("yoga_user,yoga_user_extension")->where($map)->count();	
			if( $count > 0 && $limit > 0) { 
              $total_pages = ceil($count/$limit); 
			} else { 
			              $total_pages = 0; 
			} 	
			$response = array("page"=>$page,"total"=>$total_pages,"records"=>$count,"rows"=>$employees);

			//response clubs to choose
			// $clubs = M("Club")->where(array("brand_id"=>get_brand_id()))->field('id,club_name')->select();
			// $response['userdata'] = $clubs;
			$this->ajaxReturn($response);  
	}
	...
	public function showEditAction($id)
	{
		...
		$froles=M("FinanceClub")-> where(array("user_id"=>$id))->field("club_id")->select();
		foreach ($froles as $key => $value) {
			$financeroles[]=$value['club_id'];
		} 
		$this->assign("financeroles",$financeroles);
